...
// [OUTSIDE]
    if (name.equalsIgnoreCase("Outside")) {
        ...
        return "You are standing in a dark forest filled with giant redwood trees. The air is chilly, and the cabin North of you looks very old and shaky. The porch is broken, and some of the windows have cracks in them. To your West, there is a heavy cellar door with a lock. To your East, a small path leads around to the back of the house.";
    }

    // [MAIN ENTRANCE]
    if (name.equalsIgnoreCase("Main Entrance")) {
        if (game.getTurnCount() <= 1) { // Move counter = 0 or 1 (first entry)
            return "The moment you step inside, the door thuds shut, locking out the wind. The air here is stale and smells like old paper. It is very dim, and every step you take makes the floorboards groan like they are complaining. To the West, you hear a slow, heavy snoring sound—huff... wheeze...—as if something very big is dreaming nearby. To the East of you seems to be a kitchen. To the North of you is a bathroom.";
        }
        return "The air here is stale and smells like old paper. It is very dim, and every step you take makes the floorboards groan like they are complaining. To the West, the Guest Room. To the East of you seems to be a kitchen. To the North of you is a bathroom.";
    }
    ...
    if (name.equalsIgnoreCase("Garage")) {
        String base = "The dead silence in the garage creates a sense of fear and the cold air pierces your skin. In front of you there is a broken down school bus and a Papa John’s delivery car. There is a door leading to the Kitchen to the West of you and another leading to a Storage room North of you.";
        if (!game.isHasWeapon()) {
            return base + " Upon exploring, you find a baseball bat. Take the baseball bat? “Take Baseball Bat”";
        }
        return base + " A door leading to the dining room to the West and Storage North.";
    }

    // [GUEST ROOM]
    if (name.equalsIgnoreCase("Guest Room")) {
        String base = "The air here is heavy and still, filled with tiny bits of dust dancing in the dark. Long, grey cobwebs hang from the ceiling like messy decorations. A giant bed with a thick, tattered blanket sits in the center. It looks like someone just got up from a nap, leaving a deep dent in the middle of the mattress. To the North is the living room and to the East is the Main Entrance.";
        if (!game.isHasFlashlight()) {
            return base + " Take the Flashlight? “Take Flashlight”";
        }
        return base;
    }
...
if (name.equalsIgnoreCase("Dining Room")) {
  if (game.isHasEatenFood()) {
    return "A long, grand table made of dark wood sits under a layer of grey dust. The silver platter that once held the steaming feast now sits empty, stained with grease and dark crumbs. To the South is the Kitchen, to the West is the Living Room, and to the East is the Storage room.";
  }
  return "A long, grand table made of dark wood sits under a layer of grey dust. In the very center, there is a single plate of steaming hot food that looks and smells delicious. It’s very strange because there is nobody around to cook it, and the steam rises into the air in curly, ghostly shapes. To the South is the Kitchen, to the West is the Living Room, and to the East is the Storage room. Eat the food? “Eat Food”";
}

if (name.equalsIgnoreCase("Living Room")) {
  if (game.isHasWeapon()) {
    return "Faded wallpaper peels away in long, skin-like strips, and a lone rocking chair creaks rhythmically by the cold fireplace. Suddenly, a dark figure emerges from the gloom, raising a weapon to strike. Instinctively, you swing the baseball bat with all your might. The wood connects with a dull thud, dazing the attacker just long enough for you to scramble backward and slip away into the Dinning room east of you or to the previous room south of you. You are safe, for now.";
  }
